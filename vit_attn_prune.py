...
class VisionTransformerAttnPruning(nn.Module):
    """ Vision Transformer
    ...
    def forward(self, x):
        ...
        out_pred_prob = []
        init_n = 14 * 14
        kept_indices = torch.arange(0, init_n, device=x.device).unsqueeze_(0).repeat(B, 1)
        ...
        if self.training:
            if self.distill:
                return x, features, kept_indices.detach(), None
            else:
